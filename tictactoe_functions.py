...
def do_best_turn(player_data, whose_turn):
    ...
    for turn in set(range(9)) - (player_data[0] | player_data[1]):
        ...
        elif (winner > current_winner and whose_turn == 1) or (
            winner < current_winner and whose_turn == 2
        ):
            best_choices = set()
            best_choices.add(turn)
...
def check_victory(player_data):
    for i in range(3):
        for j in [0, 1]:
            if (
                3 * i + 0 in player_data[j]
                and 3 * i + 1 in player_data[j]
                and 3 * i + 2 in player_data[j]
            ):
                return j + 1
            elif (
                i in player_data[j]
